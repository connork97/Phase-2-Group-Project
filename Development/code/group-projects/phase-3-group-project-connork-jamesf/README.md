...
Day Planner Text Adventure

Example Actions:
    Wake up - Options:  Snooze Alarm and sleep more, scroll on phone or get up and get ready?
    After getting ready - Options:  Make breakfast or skip it and go to class?
Possible Tables:
    Day of the Week Table
        Name of Day
        Day Id
        Activity ID
    Actions/Activity Table
        Action ID
        Time of Day (morning, afternoon, evening)
        Type of Action (productive, relaxing, time wasting, etc)
        Action Description
    Reminders Table
        Reminders for Tasks that are not time sensitive but need to be done and can be accessed at any time
    Friends Table
    Location Table

Tuples:
    When retrieving data from the tables

Lists:
    Storing data fetched from tables for easier use directly in Python
    Using index from lists fetched from database

Dictionary:
    Selecting an action, and storing both the time as well as selected action
